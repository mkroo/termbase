...
- 띄어쓰기가 포함된 합성어는 Elasticsearch synonym filter를 통해 하나의 용어로 통합된다 (예: "삼성 전자" → "삼성전자")
- 합성어 검색 시 띄어쓰기 유무와 관계없이 동일한 결과를 반환한다 (예: "삼성 전자" 또는 "삼성전자" 검색 시 동일 결과)
- 용어 사전에 합성어 등록 시 자동으로 synonym 규칙이 생성된다
...
**As a** 관리자
**I want to** Confluence 워크스페이스를 연동하고 모니터링할 Space를 선택할 수 있다
**So that** 해당 Space의 문서에서 용어를 수집할 수 있다

**인수조건:**

- Confluence OAuth 2.0 (3LO)을 통해 Atlassian Cloud 워크스페이스를 연동할 수 있다
- 연동된 워크스페이스의 Space 목록을 조회할 수 있다
- 수집할 Space를 다중 선택할 수 있다
- 선택한 Space의 모든 페이지(제목 + 본문)를 수동으로 수집할 수 있다
- 수집된 문서는 SourceDocument로 저장되어 기존 용어 추출 프로세스에 통합된다
- 동일한 페이지는 중복 저장되지 않는다 (페이지 ID 기반 upsert)
- 연동 상태를 확인할 수 있다
